...
2. Add a  File 
To protect build performance and avoid leaking credentials, create a  file in the root directory to stop bulky local folders from transferring to the builder context: [6, 7, 8, 9, 10]  

3. Build and Run Commands 
Execute these commands in your terminal to compile and run your containerized Bun app locally: [11, 12, 13]  
Production Best Practices Breakdown 

• Use : This forces  to respect your exactly locked  state. It blocks accidental package updates that break builds mid-pipeline. 
...
• Enforce Non-Root Execution: Adding  shifts the internal environment to a safe, low-privileged user account managed natively by the base image. 
• Handle Entrypoint Correctly: Prefer ENTRYPOINT [] over standard CMD. This guarantees that Unix termination processes () pass down perfectly to Bun for graceful application shutdowns. [1, 2, 4]
...
Always execute them in order:

# 1. Start the creator first to spin up the network
cd ../backend
docker compose up -d

# 2. Then start the consumer
cd ../frontend
docker compose up -d
